...
rem start "http-server" cmd /c npx live-server --no-browser dist

::: Ngrok to make public
rem start "ngrok" ngrok http 8080

start "http-server-ngrok" wt new-tab --title "http-server-ngrok" -d . cmd /c "title http-server-ngrok && npx live-server --no-browser dist" ; split-pane --title "http-server-ngrok" ngrok http 8080
